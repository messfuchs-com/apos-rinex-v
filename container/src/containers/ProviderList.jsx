...
import formatDate, { dateToObject } from '../share/formatDate';
...
class ProviderList extends Component {
  ...
  generateState(tempDate) {
    ...
    parseOldRinex(filePath).then((response) => {
      ...
      // console.log(`Error with file ${filePath}, ${error}`);
    ...
    if (providers && Object.keys(providers).length > 0) {
      ...
      Object.keys(providers).forEach((providerId) => {
        ...
        for (let i = 0; i < providers[providerId].stations.length; i++) {
          const station = providers[providerId].stations[i];
          const stationData = {
            sId: station.sId,
